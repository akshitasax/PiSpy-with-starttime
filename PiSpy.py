    ...
    def _takeAction(self):
        ...
        def enable_video(): #function to check if video box is checked
            if video.get() %2 != 0:#if video box is checked, enable video setting control
                self.startdate_box.config(state = NORMAL)
                self.starttime_box.config(state = NORMAL)
                ...
                self.starttime_box.config(state = DISABLED)
                self.startdate_box.config(state = DISABLED)
        ...
        def set_on(): #sets screen when recording is started
            ...
            #sleep until specified start time
            time = datetime.now().strftime("%H:%M:%S") #get start time and day of month
            timeinsec = (int(time[0])*10 + int(time[1]))*3600 + (int(time[3])*10 + int(time[4]))*60 + (int(time[6])*10 + int(time[7]))# convert current time to second
            
            starttime = self.starttime_box.get() #get given starttime in hour:minute:second
            starttimeinsec = int(starttime[0:2])*3600 + int(starttime[3:5])*60 + int(starttime[6:7]) #convert starttime to seconds
            
            moddiff = starttimeinsec-timeinsec #different between current time and given start time
            if moddiff < 0: #if difference is negative, add the no. of seconds in a day (next day)
                diff = moddiff + 86400
                sleep(diff) 
            else: #start time is on same day as current time
                sleep(moddiff)
            ...
            if timedImageMode.get() == 1 or timedVideoMode.get() == 1: #timed mode selected
                if video.get() == 1: #timed video mode selected
                    ...
                    self.captureLength = int(self.capture_box.get()[0:2])*3600 + int(self.capture_box.get()[3:5])*60 + int(self.capture_box.get()[6:7]) # record captureLength once initially and store for duration of recordings
...
#             lights.redLight('off') #turns off light
    ...
    def _setUpDisplay(self): #sets up display, creating and placing remaining buttons
        ...
        self.timecapture_description = Label(self.master, text="(hh:mm:ss)").grid(row=1, column = 2, sticky = W)
        
        self.starttime_description = Label(self.master, text= "Start Time:").grid(row = 21, column = 0, sticky = W)
        self.timestarttime_description = Label(self.master, text="(hh:mm:ss)").grid(row=21, column = 2, sticky = W)
        
        self.startdate_description = Label(self.master, text= "Start Day:").grid(row = 22, column = 0, sticky = W)
        self.timestartdate_description = Label(self.master, text="(of month)").grid(row=22, column = 2, sticky = W)
        ...
        self.startdate_box = Entry(self.master, state=DISABLED)
        self.startdate_box.grid(row = 22, column = 1, sticky = W)
        self.starttime_box = Entry(self.master, state=DISABLED)
        self.starttime_box.grid(row = 21, column = 1, sticky = W)
